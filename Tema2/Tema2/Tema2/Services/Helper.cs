...
namespace Tema2.Services
{
    public class Helper
    {
        ...
        {
            // Înlocuiește calea de mai jos cu calea exactă unde este localizat fișierul about.txt în proiectul tău
            string path = "D:\\FACULTATE\\Anul II\\Semestrul II\\MVP\\Teme\\Tema2\\Tema2\\Resources\\about.txt";

            try
            {
                using (var reader = new StreamReader(path))
                {
                    string aboutContent = reader.ReadToEnd();
                    MessageBox.Show(aboutContent, "About", MessageBoxButton.OK);
                }
            }
            catch (Exception ex)
            {
